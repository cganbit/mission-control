...
→ Máximo 5 stories por epic — se mais, dividir em 2 epics
...
```yaml
PRD_PATH: knowledge/prds/prd-NNN-nome.md    # ou convenção do projeto
EPIC: [nome]
STORIES:
  - story: [nome]
    tasks:
      - id: "1.1"
        ...
        agent: backend
        depends_on: []
        test: "comando concreto → resultado esperado"
        estimate: "~30min"
      - id: "1.2"
        title: [nome]
        agent: backend
        depends_on: ["1.1"]
        test: "curl -sS <endpoint> → 200"
        estimate: "~1h"
PARALLEL_GROUPS:
  - group: 1
    tasks: ["1.1"]
...
Salvar o PRD completo no diretório de PRDs do projeto (padrão sugerido: `knowledge/prds/`; adaptar ao consumer). O número `NNN` deve ser sequencial — verificar o maior número existente antes de criar.

**Regras do output:**
- `READY: true` → `STORIES` deve ter pelo menos 1 story com tasks
- `READY: false` → `STORIES` pode estar incompleto mas deve ter motivo em `EPIC`
- **Nunca omitir estes 4 campos** (`PRD_PATH`, `EPIC`, `STORIES`, `READY`) — o pipeline bloqueia sem eles
