...
function BarChart({ data, max }: { data: DayUsage[]; max: number }) {
  if (!data.length) return <div className="text-center text-gray-600 py-8 text-sm">Sem dados</div>;
  return (
      ...
      {data.map(d => {
        ...
        return (
          <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group relative">
            <div
              className="w-full rounded-t-sm bg-indigo-600 group-hover:bg-indigo-500 transition-colors cursor-default"
              style={{ height: `${Math.max(pct, 2)}%` }}
            />
            {/* Tooltip */}
            <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-gray-700 text-white text-xs rounded px-2 py-1 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Tokens & Custo</h1>
          <p className="text-gray-400 text-sm mt-1">Consumo de tokens por agente</p>
        </div>
        <select
          value={days}
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        ...
        {loading ? (
          <div className="h-32 flex items-center justify-center text-gray-600 text-sm">Carregando...</div>
        ) : (
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-800">
          <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Por Agente</h2>
        </div>
        {loading ? (
          <div className="text-center text-gray-600 py-12 text-sm">Carregando...</div>
        ) : !data?.byAgent.length ? (
          <div className="text-center py-16 text-gray-500">
            <div className="text-4xl mb-3">📊</div>
            <p className="text-sm">Nenhum registro ainda</p>
            <p className="text-xs text-gray-600 mt-1">Use POST /api/tokens para registrar uso</p>
          </div>
        ) : (
              ...
              return (
                <div key={a.agent_id ?? i} className="px-5 py-4">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm font-medium text-white">{a.agent_name ?? 'Desconhecido'}</span>
                    <span className="text-xs text-gray-500">{a.squad_name}</span>
                    <div className="flex-1" />
                    <span className="text-sm font-mono text-indigo-400">{fmtK(Number(a.tokens_total))}</span>
                    <span className="text-xs text-gray-500 w-16 text-right">{fmtCost(Number(a.cost_usd))}</span>
                  </div>
